...
Command: npx gltfjsx@6.5.0 venom.glb 
*/
"use client"
...
export default function Model(props) {
  ...
  const { scene, animations } = useGLTF('/venom.glb')
  const clone = React.useMemo(() => SkeletonUtils.clone(scene), [scene])
  const { nodes, materials } = useGraph(clone)
  const { actions } = useAnimations(animations, group)

  const [currentAction, setCurrentAction]=useState("move2");
  const [position, setPosition]=useState([0,-1.9,-10]);
  useEffect(() => {
    if (actions) {
  ...
  return (
    <group ref={group} {...props} dispose={null}  position={position} scale={[2,2,2]}>
      <group name="Scene">
        ...
        <group name="Armature002" position={[0.041, 0, 0]} rotation={[Math.PI / 2, 0, 0]} scale={0.01}>
          ...
          <skinnedMesh name="Object_8001" geometry={nodes.Object_8001.geometry} material={materials['material.002']} skeleton={nodes.Object_8001.skeleton} />
